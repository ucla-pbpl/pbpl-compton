...
def generate_mix(edep, num_events, energy_ranges, x_max, 
    ...
    #weights_functions = [get_weights_gaussian, get_weights_cosine, get_weights_random]
    ...
    for i in range(0, num):
        ...
        g_weights = np.stack((get_weights_gaussian(num_simulations, 20), 
                            get_weights_random(num_simulations)*0.01, 
                            #get_weights_random(num_simulations)*0.01, 
        ...
        float_weights = ((np.dot(g_weights.T, function_weights)).T)*100000
        #print(float_weights)
        weights = np.zeros(num_simulations, dtype = int)
        for j in range(0, num_simulations):
            weights[j] = max(0, int(float_weights[j])) 
        ...
        for k in range(0, len(weights)):
            es[start:start+weights[k]] = np.log(np.random.uniform(
                low=energy_ranges[k], high=energy_ranges[k+1], size=weights[k]))
            start = start+weights[k]

        assert ys.shape == es.shape, "bug generating particle es and ys"
        histo, _, _ = np.histogram2d(ys, es, 
            range=[[l_y_lower, l_y_upper],[ln_e_lower, ln_e_upper]], 
        ...
        if (i%100 == 0):
            ...
            plot_preview(new_edep, histo, data_file+"-"+random_affix+"-"+str(i)+".png")
        if (i%10 == 0): 
