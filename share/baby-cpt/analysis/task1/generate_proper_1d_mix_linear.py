...
def get_weights_random(num_simulations):
    return np.random.rand(num_simulations)*(upper-lower)+lower
...
def generate_mix(edep, num_events, energy_ranges, x_max, 
    ...
    for i in range(0, num):
        ...
        e_component_weights = np.stack((get_weights_gaussian(l_e_bins, 5),
                            get_weights_gaussian(l_e_bins, 2),
                            get_weights_gaussian(l_e_bins, 10), 
                            #get_weights_cosine(l_e_bins)*0.1, 
                            #get_weights_cosine(l_e_bins)*0.5, 
                            get_weights_gaussian(l_e_bins, 4), 
                            ))
        function_weights = np.random.rand(4)
        e_float_weights = ((np.dot(e_component_weights.T, function_weights)).T)*10000000#1e9
