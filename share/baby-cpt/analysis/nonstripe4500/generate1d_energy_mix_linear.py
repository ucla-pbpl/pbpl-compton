...
def generate_mix(edep, num_events, energy_ranges, x_max, 
    ...
    resampled_energy_ranges = np.linspace(l_e_lower, l_e_upper, l_e_bins+1)
    #np.logspace(np.log(l_e_lower), np.log(l_e_upper), l_e_bins+1, base=np.e)
    ...
    for i in range(0, num):
        ...
        e_component_weights = np.stack((#get_weights_gaussian(l_e_bins, 0.2),
                            get_weights_gaussian(l_e_bins, 50),
